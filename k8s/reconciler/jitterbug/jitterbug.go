...
func NewJitterChecker(config *Config, callable JitterFunction, logger logr.Logger) *JitterChecker {
	calculator := NewJitterCalculator(config, nil)
	return NewJitterCheckerFromCalculator(*calculator, callable, logger)
}

// NewJitterCheckerFromCalculator will complete initialization of optional Config fields and return a jitter checker
func NewJitterCheckerFromCalculator(calculator JitterCalculator, callable JitterFunction, logger logr.Logger) *JitterChecker {
	...
	return &JitterChecker{
		...
		calculator: &calculator,
		callable:   callable,
		logger:     logger.WithName("jitterbug"),
	}
}
...
func (j *JitterChecker) run() {
	...
	if j.config.InitialDelay > 0 {
		j.logger.V(1).Info("initial delay", "duration", j.config.InitialDelay)
		select {
	...
	if err != nil {
		j.logger.Error(err, "jitter check failed")
		return
	}
