...
// NewOnlineHandler creates a new online mode lifecycle.
...
func (h *OnlineHandler) NeedsRegistration(ctx context.Context, obj contract.ProductRegistrationObject) bool {
	...
	if !status.IsConditionTrue(primitives.ConditionTypeAnnounced) {
		return true
	}
	if !status.IsConditionTrue(primitives.ConditionTypeURLReady) {
		return true
	}
...
func (h *OnlineHandler) NeedsActivation(ctx context.Context, obj contract.ProductRegistrationObject) bool {
	status := obj.GetStatus()
...
func (h *OnlineHandler) ReadyForActivation(ctx context.Context, obj contract.ProductRegistrationObject) bool {
	status := obj.GetStatus()

	// Registration must have succeeded
	return status.IsConditionTrue(primitives.ConditionTypeAnnounced) &&
		status.IsConditionTrue(primitives.ConditionTypeURLReady)
}

func (h *OnlineHandler) NeedsKeepalive(ctx context.Context, obj contract.ProductRegistrationObject) bool {
	status := obj.GetStatus()
...
func (h *OnlineHandler) PrepareForRegister(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
	status := obj.GetStatus()
...
func (h *OnlineHandler) PrepareRegisteredForActivation(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
	status := obj.GetStatus()

	// Set conditions
	status.SetCondition(primitives.ConditionTypeAnnounced, metav1.ConditionTrue, "Announced", "System announced to SCC")
	status.SetCondition(primitives.ConditionTypeURLReady, metav1.ConditionTrue, "Ready", "SCC URL ready")

	return obj, nil
}

func (h *OnlineHandler) PrepareActivatedForKeepalive(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
	status := obj.GetStatus()
	...
	status.SetCondition(primitives.ConditionTypeActivated, metav1.ConditionTrue, "Activated", "Products activated with SCC")
...
func (h *OnlineHandler) PrepareKeepaliveSucceeded(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
	status := obj.GetStatus()

	// Update keepalive condition
	status.SetCondition(primitives.ConditionTypeKeepalive, metav1.ConditionTrue, "Success", "Keepalive successful")
...
func (h *OnlineHandler) Register(ctx context.Context, obj contract.ProductRegistrationObject) (int, error) {
	spec := obj.GetSpec()
	...
	regCode, err := h.fetchSecretData(ctx, regCodeRef, primitives.SecretKeyRegistrationCode)
	if err != nil {
	...
	// 8. Query subscription metadata from SCC
	/**
		subscriptionInfo, infoErr := sccClient.FetchSubscriptionInfo(ctx, regCode)
	if infoErr != nil {
		// Log warning but don't fail registration - subscription metadata is supplementary
		// TODO: Add proper logging when available
		// log.Info("Could not fetch subscription metadata", "error", infoErr)
	} else {
		// Convert scc.SubscriptionMetadata to v1.SubscriptionMetadata
		v1Info := &v1.SubscriptionMetadata{
			Kind:           subscriptionInfo.Kind,
			Name:           subscriptionInfo.Name,
			StartsAt:       subscriptionInfo.StartsAt,
			ExpiresAt:      subscriptionInfo.ExpiresAt,
			ProductClasses: make([]v1.ProductClassInfo, len(subscriptionInfo.ProductClasses)),
		}
		for i, pc := range subscriptionInfo.ProductClasses {
			v1Info.ProductClasses[i] = v1.ProductClassInfo{
				Name:        pc.Name,
				Description: pc.Description,
			}
		}

		// Cast to concrete type to set subscription info
		// (not in contract interface as it's library-specific)
		if v1Status, ok := status.(*v1.ProductRegistrationStatus); ok {
			v1Status.SetSubscriptionInfo(v1Info)
		}
	}
	*/
...
func (h *OnlineHandler) Activate(ctx context.Context, obj contract.ProductRegistrationObject) error {
	spec := obj.GetSpec()
	status := obj.GetStatus()
...
func (h *OnlineHandler) Keepalive(ctx context.Context, obj contract.ProductRegistrationObject) error {
	spec := obj.GetSpec()
	status := obj.GetStatus()
...
func (h *OnlineHandler) Deregister(ctx context.Context, obj contract.ProductRegistrationObject) error {
	spec := obj.GetSpec()
	status := obj.GetStatus()
...
func (h *OnlineHandler) ReconcileRegisterError(ctx context.Context, obj contract.ProductRegistrationObject, err error) contract.ProductRegistrationObject {
	status := obj.GetStatus()

	// Set failure condition
	status.SetCondition(primitives.ConditionTypeFailure, metav1.ConditionTrue, "RegistrationFailed", err.Error())
	status.SetCondition(primitives.ConditionTypeProgressing, metav1.ConditionFalse, "Failed", "Registration failed")

	return obj
}

func (h *OnlineHandler) ReconcileActivateError(ctx context.Context, obj contract.ProductRegistrationObject, err error) contract.ProductRegistrationObject {
	status := obj.GetStatus()

	// Set failure condition
	status.SetCondition(primitives.ConditionTypeFailure, metav1.ConditionTrue, "ActivationFailed", err.Error())
	status.SetCondition(primitives.ConditionTypeProgressing, metav1.ConditionFalse, "Failed", "Activation failed")

	return obj
}

func (h *OnlineHandler) ReconcileKeepaliveError(ctx context.Context, obj contract.ProductRegistrationObject, err error) contract.ProductRegistrationObject {
	status := obj.GetStatus()

	// Set failure condition (but don't invalidate activation)
	status.SetCondition(primitives.ConditionTypeFailure, metav1.ConditionTrue, "KeepaliveFailed", err.Error())
	status.SetCondition(primitives.ConditionTypeKeepalive, metav1.ConditionFalse, "Failed", "Keepalive failed")
...
// Based on SCC Operator's online lifecycle (always returns false).
func (h *OnlineHandler) NeedsPreprocessRegistration(ctx context.Context, obj contract.ProductRegistrationObject) bool {
...
// Based on SCC Operator's online lifecycle.
func (h *OnlineHandler) PreprocessRegistration(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
...
// Based on SCC Operator's online lifecycle.
func (h *OnlineHandler) ResetToReadyForActivation(ctx context.Context, obj contract.ProductRegistrationObject) (contract.ProductRegistrationObject, error) {
	status := obj.GetStatus()
	...
	status.SetCondition(primitives.ConditionTypeReady, metav1.ConditionFalse, "NotReady", "Registration reset")
	status.SetCondition(primitives.ConditionTypeActivated, metav1.ConditionFalse, "NotActivated", "Activation cleared")
...
func (h *OnlineHandler) createCredentialsSecret(ctx context.Context, obj contract.ProductRegistrationObject) (*corev1.SecretReference, error) {
	...
	secretName := primitives.SCCCredentialsSecretName(obj.GetName())
	...
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			...
			Finalizers: []string{
				primitives.FinalizerSCCCredentials,
			},
		},
...
func (h *OnlineHandler) fetchCredentials(ctx context.Context, ref *corev1.SecretReference) (login, password string, err error) {
	login, err = h.fetchSecretData(ctx, ref, primitives.SecretKeyLogin)
	if err != nil {
		return "", "", fmt.Errorf("failed to fetch login: %w", err)
	}

	password, err = h.fetchSecretData(ctx, ref, primitives.SecretKeyPassword)
	if err != nil {
...
func (h *OnlineHandler) updateCredentialsSecret(ctx context.Context, ref *corev1.SecretReference, login, password string) error {
	...
	secret.Data[primitives.SecretKeyLogin] = []byte(login)
	secret.Data[primitives.SecretKeyPassword] = []byte(password)
...
func (h *OnlineHandler) extractHostname(metricsData map[string]any) (string, error) {
	...
	if hostnameValue, ok := metricsData[primitives.MetricsKeyHostname]; ok {
...
func (h *OnlineHandler) determineSCCURL(spec contract.ProductRegistrationSpec) string {
...
func (h *OnlineHandler) createSCCClient(ctx context.Context, sccURL string) *scc.DefaultClient {
